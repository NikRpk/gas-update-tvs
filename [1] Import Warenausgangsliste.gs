...
function test() {
  var date = new Date();
  date.setDate(date.getDate() - 5); // Start from 5 days ago

  for (let i = 0; i < 10; i++) {
    // Clone the date to avoid modifying the original date in each iteration
    var futureDate = new Date(date);
    futureDate.setDate(date.getDate() + i); // Increment days by i in each iteration
    Logger.log(futureDate);

    var output = getCalendarWeek_(futureDate);
    Logger.log(output);
  }
}
